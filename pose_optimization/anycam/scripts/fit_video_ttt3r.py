...
from anycam.models.ttt3r_wrapper import TTT3RWrapper
...
def load_ttt3r(model_path, checkpoint=None, loaded_config=None):
    """
    Load TTT3RWrapper model from AnyCam checkpoint.
    ...
    if training_steps:
        ...
        if missing_keys:
            print(f"Missing keys (expected for TTT3RWrapper): {missing_keys}")
        if unexpected_keys:
...
def fit_video_ttt3r_wrapper(config, model, criterion, imgs, device, ttt3r_data_path=None, seq_name=None, gt_proj=None):
    """Wrapper function to maintain compatibility with original interface"""
    return fit_video_ttt3r(config=config, model=model, criterion=criterion, imgs=imgs, device=device, ttt3r_data_path=ttt3r_data_path, seq_name=seq_name, gt_proj=gt_proj, return_extras=False)


@torch.autocast(device_type="cuda", enabled=True)
@torch.no_grad()
def fit_video_ttt3r(config, model, criterion, imgs, device="cuda", 
                   return_extras=False, gt_proj=None, ttt3r_data_path=None, seq_name=None):
    """
    Main function for video fitting using TTT3R predictions.
    """
    print(seq_name)
    print(config)

    if ttt3r_data_path is None:
        raise ValueError("ttt3r_data_path must be provided")
    if seq_name is None:
    ...
    if config.get("with_rerun", False):
        rr.init("TTT3R Prediction", recording_id=uuid.uuid4())
        rr.connect()
    ...
    logger.info("Loading TTT3R predictions")
    
    # Load TTT3R predictions from output folder
    ttt3r_image_size = None
    ttt3r_data = model.load_ttt3r_predictions(ttt3r_data_path, seq_name)
    logger.info(f"Loaded TTT3R data: poses {ttt3r_data['poses'].shape}, intrinsics {ttt3r_data['intrinsics'].shape}")
    
    ttt3r_image_size = ttt3r_data.get('image_size', None)
    if ttt3r_image_size is not None:
        ttt3r_width, ttt3r_height = ttt3r_image_size
        logger.info(f"TTT3R intrinsics calibrated for: {ttt3r_width}x{ttt3r_height}")
    else:
        logger.warning("TTT3R image size not available, assuming intrinsics match dataset images")
    ...
    # Rescale intrinsics if dataset image size differs from TTT3R's calibrated size
    if ttt3r_image_size is not None and ttt3r_width > 0 and ttt3r_height > 0:
        dataset_width, dataset_height = w, h
        if ttt3r_width != dataset_width or ttt3r_height != dataset_height:
            logger.info(f"Rescaling intrinsics from TTT3R size {ttt3r_width}x{ttt3r_height} to dataset size {dataset_width}x{dataset_height}")
            ttt3r_max_dim = max(ttt3r_width, ttt3r_height)
            dataset_max_dim = max(dataset_width, dataset_height)
            resize_ratio = dataset_max_dim / ttt3r_max_dim
            rescaled_intrinsics = ttt3r_data['intrinsics'].clone()
            ...
            ttt3r_data['intrinsics'] = rescaled_intrinsics
    ...
    # Compute depth and flow using the model; use TTT3R disparities if available
    seq_imgs, seq_depths, seq_flow_occs_fwd, seq_flow_occs_bwd = compute_depth_flow(
        model, seq_imgs, megasam_disps=ttt3r_data.get('disps')
    )
    
    # Convert TTT3R poses to trajectory
    ttt3r_poses = ttt3r_data['poses'].cpu().numpy()
    best_trajectory = []
    for i in range(len(ttt3r_poses)):
        pose_matrix = torch.tensor(ttt3r_poses[i], dtype=torch.float32)
        best_trajectory.append(pose_matrix.numpy())
        if i < 3:
            print(f"TTT3R pose {i} (camera-to-world):")
            print(pose_matrix.numpy())
            print()
    if len(best_trajectory) != len(dataset.imgs):
        best_trajectory = best_trajectory[:len(dataset.imgs)]
    
    # Projection matrix from TTT3R intrinsics
    ttt3r_intrinsics = ttt3r_data['intrinsics'].cpu().numpy()
    proj = ttt3r_intrinsics.copy()
    
    # Use confidence maps as uncertainties if available
    motion_prob = ttt3r_data['motion_prob']
    motion_prob = motion_prob.cpu().numpy() if motion_prob is not None else None
    
    if motion_prob is not None and len(motion_prob) != len(dataset.imgs):
    ...
    if do_ba_refinement:
        logger.info("Starting BA refinement with TTT3R initial trajectory")
        
        if ba_refinement_level > 1:
            imgs0 = dataset.imgs[:-1:ba_refinement_level]
            imgs1 = dataset.imgs[1::ba_refinement_level]
            ttt3r_disps_ba = None
            if ttt3r_data.get('disps') is not None:
                ttt3r_disps_ba = ttt3r_data['disps'][:-1:ba_refinement_level]
            seq_imgs_ba, seq_depths_ba, seq_flow_occs_fwd_ba, seq_flow_occs_bwd_ba = compute_depth_flow(
                model, imgs0=imgs0, imgs1=imgs1, megasam_disps=ttt3r_disps_ba
            )
