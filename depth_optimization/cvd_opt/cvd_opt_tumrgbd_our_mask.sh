...
for seq in ${evalset[@]}; do
  ...
  --scene_name $seq \
  --output_dir outputs_cvd_tumrgbd_our_mask_opt_uncert \
  --w_grad 2.0 \
  --w_normal 5.0 \
  --mask_path /data/zhuoyuan/code/Sa2VA/outputs/SA2VA_8B_tumrgbd_uniform_keyframe_selector_base64_sequence_num_samples_64 \
  --opt_uncert true \
  --device cpu
done
