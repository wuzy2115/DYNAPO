...
class SintelDataset(Dataset):
    ...
    #             seq_len = len(os.listdir(os.path.join(data_path, seq, 'rgb')))
    #             sequences[seq] = seq_len
    #     return sequences
    @staticmethod
    ...
    #                 cv2.imread(
    #                     os.path.join(
    #                         self.data_path, seq, "rgb", f"{id:05d}.png"
    #                     )
    #                 ),
    #                 cv2.COLOR_BGR2RGB,
    #             ).astype(np.float32)
    ...
    #     depth_path = os.path.join(self.data_path, seq, "depth", f"{id:05d}.npy")
    #     depth = np.float32(np.load(depth_path))
    #     depth[depth > 1000] = 0
    ...
    #     cam_path = os.path.join(self.data_path, sequence, "extrinsics.npy")
    #     calib_path = os.path.join(self.data_path, sequence, "calibration.txt")
    #     gt_cam2w = np.load(cam_path)
    #     poses = gt_cam2w[ids]
        
    #     # poses = np.array([np.linalg.inv(pose) for pose in gt_cam2w[ids]])
    #     # for pose in gt_cam2w:
    #     #     R = pose[:3, :3].copy()
    ...
    #     #         Vt[2, :] *= -1
    #     #         R_ortho = U @ Vt
    #     #     pose[:3, :3] = R_ortho  # Override with orthogonalized rotation

    #     fx, fy, cx, cy = np.loadtxt(calib_path).tolist()
    #     K = np.eye(3)
    #     K[0, 0], K[1, 1], K[0, 2], K[1, 2] = fx, fy, cx, cy
    #     projs = np.tile(K[np.newaxis, :, :], (len(ids), 1, 1))
    #     return projs, poses
