...
class Cut3RWrapper(nn.Module):
    """
    Wrapper class for integrating CUT3R predictions into AnyCam pipeline.
    This class loads precomputed CUT3R results written by ref_cut3r/demo.py.
    """
    ...
    def load_cut3r_predictions(self, cut3r_data_path, seq_name):
        """
        Load CUT3R predictions from the output directory structure created by ref_cut3r/demo.py.

        Layout expected:
          <cut3r_data_path>/<seq_name>/
        ...
        """
        root = Path(cut3r_data_path) / seq_name
        ...
        if not camera_dir.exists():
            raise FileNotFoundError(f"CUT3R camera directory not found: {camera_dir}")
        ...
        if image_width is None or image_height is None:
            logger.warning("CUT3R image size not available; defaulting to (0, 0)")
            image_width, image_height = 0, 0

        logger.info(f"Loaded CUT3R data: poses {poses.shape}, intrinsics {intrinsics.shape}")
        return {
            'poses': poses,
    ...
    def forward(self, x):
        raise NotImplementedError("Cut3RWrapper does not support forward pass. Use load_cut3r_predictions instead.")
