...
def load_video_frames(
    ...
    def _extract_frame_index(filename):
        """Extracts the last integer in the filename before the extension."""
        name = os.path.splitext(filename)[0]
        matches = list(re.finditer(r'\d+', name))
        if not matches:
            raise ValueError(f"Cannot extract frame index from filename: {filename}")
    ...
    frame_names.sort(key=_extract_frame_index)
