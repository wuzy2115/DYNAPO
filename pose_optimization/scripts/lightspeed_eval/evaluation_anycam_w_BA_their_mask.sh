...
#     ++fit_video.prediction.mask_path=/data/zhuoyuan/sintel_orig/Annotations >> exps_BA/lightspeed_megasam_wo_BA_full.txt


#  anycam + BA
CUDA_VISIBLE_DEVICES=6 python anycam/scripts/evaluate_trajectories.py \
    -cn evaluate_trajectories \
    ++model_path=pretrained_models/anycam_seq8 \
    ++dataset=anycam/configs/dataset_cfgs/lightspeed.yaml \
    ++fit_video.dataset_type=lightspeed \
    ...
    ++fit_video.ba_refinement.lr=1e-5 \
    ...
    ++fit_video.prediction.flow_model=unimatch \
    ++fit_video.prediction.depth_predictor=unidepth \
    ++fit_video.prediction.mask_path=/data/zhuoyuan/code/Sa2VA/outputs/SA2VA_8B_lightspeed_uniform_keyframe_selector_num_samples_8 >> exps_BA/lightspeed_anycam_BA_full_their_mask_lr_1e-5.txt