...
def save_full_reconstruction(
    droid, full_traj, rgb_list, senor_depth_list, motion_prob, scene_name, dataset_type
):
  ...
  Path("reconstructions_{}/{}".format(dataset_type, scene_name)).mkdir(
      parents=True, exist_ok=True
  )
  np.save("reconstructions_{}/{}/images.npy".format(dataset_type, scene_name), images)
  np.save("reconstructions_{}/{}/disps.npy".format(dataset_type, scene_name), disps)
  np.save("reconstructions_{}/{}/poses.npy".format(dataset_type, scene_name), poses)
  np.save(
      "reconstructions_{}/{}/intrinsics.npy".format(dataset_type, scene_name), intrinsics * 8.0
  )
  np.save("reconstructions_{}/{}/motion_prob.npy".format(dataset_type, scene_name), motion_prob)
  ...
  print("outputs_%s/%s_droid.npz" % (dataset_type, scene_name))
  Path("outputs_%s" % dataset_type).mkdir(parents=True, exist_ok=True)

  np.savez(
      "outputs_%s/%s_droid.npz" % (dataset_type, scene_name),
...
if __name__ == "__main__":
  ...
  parser.add_argument("--dataset_type", default="davis")
