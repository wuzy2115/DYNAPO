...
from anycam.models.easi3r_wrapper import Easi3RWrapper
...
def load_easi3r(model_path, checkpoint=None, loaded_config=None):
    """
    Load Easi3RWrapper model from AnyCam checkpoint.
    We only use it as a wrapper to load precomputed predictions.
    """
    ...
    model = Easi3RWrapper(model_conf)
...
def fit_video_easi3r_wrapper(config, model, criterion, imgs, device, easi3r_data_path=None, seq_name=None, gt_proj=None):
    return fit_video_easi3r(config=config, model=model, criterion=criterion, imgs=imgs, device=device, easi3r_data_path=easi3r_data_path, seq_name=seq_name, gt_proj=gt_proj, return_extras=False)


@torch.autocast(device_type="cuda", enabled=True)
@torch.no_grad()
def fit_video_easi3r(config, model, criterion, imgs, device="cuda", 
                     return_extras=False, gt_proj=None, easi3r_data_path=None, seq_name=None):
    """
    Main function for video fitting using Easi3R predictions.
    Loads precomputed poses/intrinsics/depth/confidence from Easi3R outputs.
    """
    print(seq_name)
    print(config)

    if easi3r_data_path is None:
        raise ValueError("easi3r_data_path must be provided")
    if seq_name is None:
    ...
    if config.get("with_rerun", False):
        rr.init("Easi3R Prediction", recording_id=uuid.uuid4())
        rr.connect()
    ...
    logger.info("Loading Easi3R predictions")
    easi3r_data = model.load_easi3r_predictions(easi3r_data_path, seq_name)

    # Rescale intrinsics if dataset image size differs from Easi3R calibrated size
    easi3r_image_size = easi3r_data.get('image_size', None)
    if easi3r_image_size is not None:
        e_width, e_height = easi3r_image_size
        c, h, w = dataset.imgs.shape[1:]
        dataset_width, dataset_height = w, h
        if e_width != dataset_width or e_height != dataset_height:
            logger.info(f"Rescaling intrinsics from Easi3R size {e_width}x{e_height} to dataset size {dataset_width}x{dataset_height}")
            e_max_dim = max(e_width, e_height)
            dataset_max_dim = max(dataset_width, dataset_height)
            resize_ratio = dataset_max_dim / e_max_dim
            rescaled_intrinsics = easi3r_data['intrinsics'].clone()
            ...
            easi3r_data['intrinsics'] = rescaled_intrinsics
    ...
    seq_imgs, seq_depths, seq_flow_occs_fwd, seq_flow_occs_bwd = compute_depth_flow(
        model, seq_imgs, megasam_disps=easi3r_data.get('disps')
    )

    # Convert Easi3R poses to trajectory (camera-to-world)
    easi3r_poses = easi3r_data['poses'].cpu().numpy()
    best_trajectory = []
    for i in range(len(easi3r_poses)):
        pose_matrix = torch.tensor(easi3r_poses[i], dtype=torch.float32)
    ...
    proj = easi3r_data['intrinsics'].cpu().numpy()

    # Use motion_prob (masks or confidence) as uncertainties if available
    motion_prob = easi3r_data.get('motion_prob')
    if motion_prob is not None:
        if isinstance(motion_prob, torch.Tensor):
            motion_prob_tensor = motion_prob
        else:
            motion_prob_tensor = torch.tensor(motion_prob, dtype=torch.float32)
        # align length with number of images
        n_imgs = len(dataset.imgs)
        if motion_prob_tensor.dim() == 3:
            # (T, H, W)
            if motion_prob_tensor.shape[0] != n_imgs:
                motion_prob_tensor = motion_prob_tensor[:n_imgs]
            ba_uncertainties_tensor = motion_prob_tensor.unsqueeze(1)  # (T,1,H,W)
        elif motion_prob_tensor.dim() == 4:
            # (T,1,H,W)
            if motion_prob_tensor.shape[0] != n_imgs:
                motion_prob_tensor = motion_prob_tensor[:n_imgs]
            ba_uncertainties_tensor = motion_prob_tensor
        elif motion_prob_tensor.dim() == 1:
            # (T,)
            if motion_prob_tensor.shape[0] != n_imgs:
                motion_prob_tensor = motion_prob_tensor[:n_imgs]
            ba_uncertainties_tensor = motion_prob_tensor.view(n_imgs, 1, 1, 1).expand(n_imgs, 1, h, w)
        else:
            ba_uncertainties_tensor = torch.ones((len(seq_imgs), 1, h, w), dtype=torch.float32)
    else:
        ba_uncertainties_tensor = torch.ones((len(seq_imgs), 1, h, w), dtype=torch.float32)

    # Resize uncertainties to match seq_imgs if needed
    ...
    if do_ba_refinement:
        logger.info("Starting BA refinement with Easi3R initial trajectory")
        ...
        if ba_refinement_level > 1:
            ...
            disps_ba = None
            if easi3r_data.get('disps') is not None:
                disps_ba = easi3r_data['disps'][:-1:ba_refinement_level]
