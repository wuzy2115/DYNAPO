...
CUDA_VISIBLE_DEVICES=3 python anycam/scripts/evaluate_trajectories.py \
    ...
    ++dataset=anycam/configs/dataset_cfgs/lightspeed.yaml \
    ++fit_video.dataset_type=lightspeed \
    ++fit_video.prediction.recon_data_path=/data2/zhuoyuan/cvpr2026/megasam \
    ...
    ++fit_video.prediction.flow_model=unimatch \
    ++fit_video.prediction.depth_predictor=unidepth \
    ++fit_video.prediction.mask_path=/data/zhuoyuan/code/Sa2VA/outputs/SA2VA_8B_lightspeed_uniform_keyframe_selector_num_samples_8 >> exps_BA/lightspeed_megasam_BA_full_their_mask.txt